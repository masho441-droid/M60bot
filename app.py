...
@app.route("/")
def home():
    return "🐉 M60 - Absolute Sniper is running flawlessly", 200
...
MIN_CHANGE = 0.6                
ALERT_COOLDOWN = 900            
SYMBOLS_LIMIT = 250 # تقليص العدد لتركيز القوة الحسابية على الأسهم الأهم
...
# ====================== FETCH SYMBOLS (المعدل لصيد البري ماركت) =============
async def fetch_active_symbols(session, is_premarket):
    url = "https://scanner.tradingview.com/america/scan"
    
    # الحل المثالي: تغيير الترتيب (Sort) بناءً على وضع السوق الحالي لضمان عدم جلب أسهم ميتة
    ...
    payload = {
        ...
        "columns": ["name", "close", "volume", "premarket_change"],
        "sort": {"sortBy": sort_column, "sortOrder": "desc"} # جلب الأعلى صعوداً وحركة الآن!
    }
    try:
        ...
        print(f"❌ فشل سكنر TradingView: {e}")
        return []
...
async def main_loop():
    ...
    await send_telegram("🚀 *M60 Absolute Sniper - تم تطبيق الحلول المثالية للبري ماركت بنجاح*")
    ...
    while True:
        try:
            ...
            async with aiohttp.ClientSession() as session:
                # نمرر حالة السوق للسكنر ليقوم بالترتيب الذكي
                ...
                for symbol, data in all_data.items():
                    # دمج الفلتر النسبي مع قفل الأمان المالي الجديد (data["volume"] >= abs_min_volume)
                    ...
                    if is_potential_explosion:
                        ...
                        if final_price > data["sma20"] and can_alert(symbol):
                            ...
                            # حساب الأهداف بناءً على السعر الحقيقي لتجنب الـ Spread الواسع
                            target1, target2 = final_price * 1.05, final_price * 1.10
                            stop_loss = final_price * 0.96

                            msg = (
                                f"💥 *تنبيه ذكي - انفجار سيولة حقيقي*\n"
                                f"الوضع الحالي: {market_status}\n\n"
                                f"📊 الرمز: `{symbol}`\n"
                                f"💰 السعر اللحظي: `${final_price:.2f}`\n"
                                f"📈 حجم الشمعة الحالي: `{data['volume']:,}` سهم\n"
                                f"🚀 القفزة النسبية: `{data['volume_spike']:.1f}x` أضعاف العادي\n"
                                f"⚡ تسارع التدفق: `{data['volume_acceleration']:.1f}x`\n"
                                f"📈 صعود الشمعة: `+{data['price_change']:.2f}%`\n"
                                f"🎯 الأهداف: `{target1:.2f}` ← `{target2:.2f}`\n"
                                f"🛑 وقف الأمان: `{stop_loss:.2f}`\n"
                                f"🔢 تنبيه رقم #{alert_counters[symbol]}\n\n"
                                f"⚠️ نظام حماية وتقييم السيولة اللحظية مُفعّل"
                            )
                            await send_telegram(msg)
                            await asyncio.sleep(1)

                print(f"⏳ دورة فحص ناجحة بآلية الترتيب الديناميكي. انتظار دقيقتين...")
